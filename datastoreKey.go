...
import (
	...
	"strconv"
	"strings"
	"github.com/clementdenis/datastore-key/datastorekey"
	"bufio"
	"regexp"
)
...
func process() (string, error) {
	...
	case "replace":
		...
		for scanner.Scan() {
			line := scanner.Text()
			os.Stdout.WriteString(replaceKey(line, compiled, field) + "\n")
		}
		if err != nil {
			return "", err
		}
...
func replaceKey(line string, compiled *regexp.Regexp, field string) string {
	return compiled.ReplaceAllStringFunc(line, func(matched string) string{
		encodedKey := compiled.FindStringSubmatch(matched)[1]
		decodedKey, err := datastore.DecodeKey(encodedKey)
		if err != nil {
			return matched
		}
		fieldValue, err := extract(decodedKey, field)
		if err != nil {
			println("error 2")
			return matched
		}
		return strings.Replace(matched, encodedKey, fieldValue, 1)
	})
}
